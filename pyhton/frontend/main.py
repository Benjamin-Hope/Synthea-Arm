...
threading.Thread(target=play_loading_video, args=(loading_window, loading_video_path, '-LOADING-', 0.7), daemon=True).start()
...
layout = [
    [sg.Column([[sg.Image(key='-VIDEO-')]], justification='center', background_color='#1a1f1d'),],
...
threading.Thread(target=play_video, args=(main_window, video_path, main_window['-VIDEO-']), daemon=True).start()
